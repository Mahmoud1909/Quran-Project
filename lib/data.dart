import 'package:quran_project/model/azkar_models.dart';
import 'package:quran_project/model/category.dart';

final List<Category> categories = [
  // فئة "أذكار الصباح"
  Category(
    title: "أذكار الصباح",
    azkarList: [
      // العناصر الموجودة في قائمة متداخلة في JSON تم تسطيحها:
      Azkar(
        category: "أذكار الصباح",
        count: "1",
        description: "",
        reference: "",
        content:
        "أَصْـبَحْنا وَأَصْـبَحَ المُـلْكُ لله وَالحَمدُ لله ، لا إلهَ إلاّ اللّهُ وَحدَهُ لا شَريكَ لهُ، لهُ المُـلكُ ولهُ الحَمْـد، وهُوَ على كلّ شَيءٍ قدير",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "1",
        description:
        "من قالها موقنا بها حين يمسى ومات من ليلته دخل الجنة وكذلك حين يصبح.",
        reference: "",
        content:
        "اللّهـمَّ أَنْتَ رَبِّـي لا إلهَ إلاّ أَنْتَ ، خَلَقْتَنـي وَأَنا عَبْـدُك ، وَأَنا عَلـى عَهْـدِكَ وَوَعْـدِكَ ما اسْتَـطَعْـت ، أَعـوذُبِكَ مِنْ شَـرِّ ما صَنَـعْت ، أَبـوءُ لَـكَ بِنِعْـمَتِـكَ عَلَـيَّ وَأَبـوءُ بِذَنْـبي فَاغْفـِرْ لي فَإِنَّـهُ لا يَغْـفِرُ الذُّنـوبَ إِلاّ أَنْتَ .",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "3",
        description:
        "من قالها حين يصبح وحين يمسى كان حقا على الله أن يرضيه يوم القيامة.",
        reference: "",
        content:
        "رَضيـتُ بِاللهِ رَبَّـاً وَبِالإسْلامِ ديـناً وَبِمُحَـمَّدٍ صلى الله عليه وسلم نَبِيّـاً.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "4",
        description: "من قالها أعتقه الله من النار.",
        reference: "",
        content:
        "اللّهُـمَّ إِنِّـي أَصْبَـحْتُ أُشْـهِدُك ، وَأُشْـهِدُ حَمَلَـةَ عَـرْشِـك ، وَمَلَائِكَتَكَ ، وَجَمـيعَ خَلْـقِك ، أَنَّـكَ أَنْـتَ اللهُ لا إلهَ إلاّ أَنْـتَ وَحْـدَكَ لا شَريكَ لَـك.",
      ),
      // باقي العناصر الخاصة بفئة "أذكار الصباح"
      Azkar(
        category: "أذكار الصباح",
        count: "7",
        description:
        "من قالها كفاه الله ما أهمه من أمر الدنيا والأخرة.",
        reference: "",
        content:
        "حَسْبِـيَ اللّهُ لا إلهَ إلاّ هُوَ عَلَـيهِ تَوَكَّـلتُ وَهُوَ رَبُّ العَرْشِ العَظـيم.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "3",
        description: "لم يضره من الله شيء.",
        reference: "",
        content:
        "بِسـمِ اللهِ الذي لا يَضُـرُّ مَعَ اسمِـهِ شَيءٌ في الأرْضِ وَلا في السّمـاءِ وَهـوَ السّمـيعُ العَلـيم.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "1",
        description: "",
        reference: "",
        content:
        "اللّهُـمَّ بِكَ أَصْـبَحْنا وَبِكَ أَمْسَـينا ، وَبِكَ نَحْـيا وَبِكَ نَمُـوتُ وَإِلَـيْكَ النُّـشُور.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "1",
        description: "",
        reference: "",
        content:
        "أَصْبَـحْـنا عَلَى فِطْرَةِ الإسْلاَمِ، وَعَلَى كَلِمَةِ الإِخْلاَصِ، وَعَلَى دِينِ نَبِيِّنَا مُحَمَّدٍ صَلَّى اللهُ عَلَيْهِ وَسَلَّمَ، وَعَلَى مِلَّةِ أَبِينَا إبْرَاهِيمَ حَنِيفاً مُسْلِماً وَمَا كَانَ مِنَ المُشْرِكِينَ.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "3",
        description: "",
        reference: "",
        content:
        "سُبْحـانَ اللهِ وَبِحَمْـدِهِ عَدَدَ خَلْـقِه ، وَرِضـا نَفْسِـه ، وَزِنَـةَ عَـرْشِـه ، وَمِـدادَ كَلِمـاتِـه.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "3",
        description: "",
        reference: "",
        content:
        "اللّهُـمَّ عافِـني في بَدَنـي ، اللّهُـمَّ عافِـني في سَمْـعي ، اللّهُـمَّ عافِـني في بَصَـري ، لا إلهَ إلاّ أَنْـتَ.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "3",
        description: "",
        reference: "",
        content:
        "اللَّهُمَّ إِنَّا نَعُوذُ بِكَ مِنْ أَنْ نُشْرِكَ بِكَ شَيْئًا نَعْلَمُهُ ، وَنَسْتَغْفِرُكَ لِمَا لَا نَعْلَمُهُ.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "100",
        description:
        "كانت له عدل عشر رقاب، وكتبت له مئة حسنة، ومحيت عنه مئة سيئة، وكانت له حرزا من الشيطان.",
        reference: "",
        content:
        "لَا إلَه إلّا اللهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلُّ شَيْءِ قَدِيرِ.",
      ),
      // استبعاد عنصر "stop" إن وجد
      Azkar(
        category: "أذكار الصباح",
        count: "3",
        description: "",
        reference: "",
        content:
        "أَعـوذُ بِكَلِمـاتِ اللّهِ التّـامّـاتِ مِنْ شَرِّ ما خَلَـق.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "10",
        description:
        "من صلى على حين يصبح وحين يمسى ادركته شفاعتى يوم القيامة.",
        reference: "",
        content:
        "اللَّهُمَّ صَلِّ وَسَلِّمْ وَبَارِكْ على نَبِيِّنَا مُحَمَّد.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "3",
        description: "",
        reference: "",
        content:
        "اللَّهُمَّ إِنَّا نَعُوذُ بِكَ مِنْ أَنْ نُشْرِكَ بِكَ شَيْئًا نَعْلَمُهُ ، وَنَسْتَغْفِرُكَ لِمَا لَا نَعْلَمُهُ.",
      ),
      Azkar(
        category: "أذكار الصباح",
        count: "3",
        description: "",
        reference: "",
        content:
        "اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنْ الْهَمِّ وَالْحَزَنِ، وَأَعُوذُ بِكَ مِنْ الْعَجْزِ وَالْكَسَلِ، وَأَعُوذُ بِكَ مِنْ الْجُبْنِ وَالْبُخْلِ، وَأَعُوذُ بِكَ مِنْ غَلَبَةِ الدَّيْنِ، وَقَهْرِ الرِّجَالِ.",
      ),
    ],
  ),
  // فئة "أذكار المساء"
  Category(
    title: "أذكار المساء",
    azkarList: [
      Azkar(
        category: "أذكار المساء",
        count: "01",
        description: "",
        reference: "",
        content:
        "أَمْسَيْـنا وَأَمْسـى المـلكُ لله وَالحَمدُ لله ، لا إلهَ إلاّ اللّهُ وَحدَهُ لا شَريكَ لهُ، لهُ المُـلكُ ولهُ الحَمْـد، وهُوَ على كلّ شَيءٍ قدير، رَبِّ أسْـأَلُـكَ خَـيرَ ما في هـذهِ اللَّـيْلَةِ وَخَـيرَ ما بَعْـدَهـا ، وَأَعـوذُ بِكَ مِنْ شَـرِّ ما في هـذهِ اللَّـيْلةِ وَشَرِّ ما بَعْـدَهـا ، رَبِّ أَعـوذُبِكَ مِنَ الْكَسَـلِ وَسـوءِ الْكِـبَر ، رَبِّ أَعـوذُ بِكَ مِنْ عَـذابٍ في النّـارِ وَعَـذابٍ في القَـبْر.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "01",
        description:
        "من قالها موقنا بها حين يمسى ومات من ليلته دخل الجنة وكذلك حين يصبح.",
        reference: "",
        content:
        "اللّهـمَّ أَنْتَ رَبِّـي لا إلهَ إلاّ أَنْتَ ، خَلَقْتَنـي وَأَنا عَبْـدُك ، وَأَنا عَلـى عَهْـدِكَ وَوَعْـدِكَ ما اسْتَـطَعْـت ، أَعـوذُبِكَ مِنْ شَـرِّ ما صَنَـعْت ، أَبـوءُ لَـكَ بِنِعْـمَتِـكَ عَلَـيَّ وَأَبـوءُ بِذَنْـبي فَاغْفـِرْ لي فَإِنَّـهُ لا يَغْـفِرُ الذُّنـوبَ إِلاّ أَنْتَ .",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "03",
        description:
        "من قالها حين يصبح وحين يمسى كان حقا على الله أن يرضيه يوم القيامة.",
        reference: "",
        content:
        "رَضيـتُ بِاللهِ رَبَّـاً وَبِالإسْلامِ ديـناً وَبِمُحَـمَّدٍ صلى الله عليه وسلم نَبِيّـاً.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "04",
        description: "من قالها أعتقه الله من النار.",
        reference: "",
        content:
        "اللّهُـمَّ إِنِّـي أَمسيتُ أُشْـهِدُك ، وَأُشْـهِدُ حَمَلَـةَ عَـرْشِـك ، وَمَلَائِكَتَكَ ، وَجَمـيعَ خَلْـقِك ، أَنَّـكَ أَنْـتَ اللهُ لا إلهَ إلاّ أَنْـتَ وَحْـدَكَ لا شَريكَ لَـك.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "01",
        description: "من قالها حين يمسى أدى شكر يومه.",
        reference: "",
        content:
        "اللّهُـمَّ ما أَمسى بي مِـنْ نِعْـمَةٍ أَو بِأَحَـدٍ مِـنْ خَلْـقِك ، فَمِـنْكَ وَحْـدَكَ لا شريكَ لَـك ، فَلَـكَ الْحَمْـدُ وَلَـكَ الشُّكْـر.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "07",
        description:
        "من قالها كفاه الله ما أهمه من أمر الدنيا والأخرة.",
        reference: "",
        content:
        "حَسْبِـيَ اللّهُ لا إلهَ إلاّ هُوَ...",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "03",
        description: "لم يضره من الله شيء.",
        reference: "",
        content:
        "بِسـمِ اللهِ الذي لا يَضُـرُّ مع اسمِهِ شَيءٌ في الأرْضِ ولا في السّمـاءِ...",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "01",
        description: "",
        reference: "",
        content:
        "اللّهُـمَّ بِكَ أَمْسَـينا وَبِكَ أَصْـبَحْنا، وَبِكَ نَحْـيا وَبِكَ نَمُـوتُ وَإِلَـيْكَ الْمَصِيرُ.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "01",
        description: "",
        reference: "",
        content:
        "أَمْسَيْنَا عَلَى فِطْرَةِ الإسْلاَمِ، وَعَلَى كَلِمَةِ الإِخْلاَصِ، وَعَلَى دِينِ نَبِيِّنَا مُحَمَّدٍ صَلَّى اللهُ عَلَيْهِ وَسَلَّمَ، وَعَلَى مِلَّةِ أَبِينَا إبْرَاهِيمَ حَنِيفاً مُسْلِماً وَمَا كَانَ مِنَ المُشْرِكِينَ.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "03",
        description: "",
        reference: "",
        content:
        "سُبْحـانَ اللهِ وَبِحَمْـدِهِ عَدَدَ خَلْـقِه ، وَرِضـا نَفْسِـه ، وَزِنَـةَ عَـرْشِـه ، وَمِـدادَ كَلِمـاتِـه.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "03",
        description: "",
        reference: "",
        content:
        "اللّهُـمَّ عافِـني في بَدَنـي ، اللّهُـمَّ عافِـني في سَمْـعي ، اللّهُـمَّ عافِـني في بَصَـري ، لا إلهَ إلاّ أَنْـتَ.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "03",
        description: "",
        reference: "",
        content:
        "اللَّهُمَّ إِنَّا نَعُوذُ بِكَ مِنْ أَنْ نُشْرِكَ بِكَ شَيْئًا نَعْلَمُهُ ، وَنَسْتَغْفِرُكَ لِمَا لَا نَعْلَمُهُ.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "01",
        description: "",
        reference: "",
        content:
        "اللّهُـمَّ إِنِّـي أسْـأَلُـكَ العَـفْوَ وَالعـافِـيةَ في الدُّنْـيا وَالآخِـرَة ، اللّهُـمَّ إِنِّـي أسْـأَلُـكَ العَـفْوَ وَالعـافِـيةَ في ديني وَدُنْـيايَ وَأهْـلي وَمالـي ، اللّهُـمَّ اسْتُـرْ عـوْراتي وَآمِـنْ رَوْعاتـي ، اللّهُـمَّ احْفَظْـني مِن بَـينِ يَدَيَّ وَمِن خَلْـفـي وَعَن يَمـيني وَعَن شِمـالي ، وَمِن فَوْقـي ، وَأَعـوذُ بِعَظَمَـتِكَ أَن أُغْـتالَ مِن تَحْتـي.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "03",
        description: "",
        reference: "",
        content:
        "يَا حَيُّ يَا قيُّومُ بِرَحْمَتِكَ أسْتَغِيثُ أصْلِحْ لِي شَأنِي كُلَّهُ وَلاَ تَكِلْنِي إلَى نَفْسِي طَـرْفَةَ عَيْنٍ.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "01",
        description: "",
        reference: "",
        content:
        "أَمْسَيْنا وَأَمْسَى الْمُلْكُ للهِ رَبِّ الْعَالَمَيْنِ، اللَّهُمَّ إِنَّي أسْأَلُكَ خَيْرَ هَذَه اللَّيْلَةِ فَتْحَهَا ونَصْرَهَا، ونُوْرَهَا وبَرَكَتهَا، وَهُدَاهَا، وَأَعُوذُ بِكَ مِنْ شَرِّ مَا فيهِا وَشَرَّ مَا بَعْدَهَا.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "01",
        description: "",
        reference: "",
        content:
        "اللّهُـمَّ عالِـمَ الغَـيْبِ وَالشّـهادَةِ فاطِـرَ السّماواتِ وَالأرْضِ رَبَّ كـلِّ شَـيءٍ وَمَليـكَه ، أَشْهَـدُ أَنْ لا إِلـهَ إِلاّ أَنْت ، أَعـوذُ بِكَ مِن شَـرِّ نَفْسـي وَمِن شَـرِّ الشَّيْـطانِ وَشِرْكِهِ ، وَأَنْ أَقْتَـرِفَ عَلـى نَفْسـي سوءاً أَوْ أَجُـرَّهُ إِلـى مُسْـلِم.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "03",
        description: "",
        reference: "",
        content:
        "أَعـوذُ بِكَلِمـاتِ اللّهِ التّـامّـاتِ مِنْ شَرِّ ما خَلَـق.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "10",
        description:
        "من صلى على حين يصبح وحين يمسى ادركته شفاعتى يوم القيامة.",
        reference: "",
        content:
        "اللَّهُمَّ صَلِّ وَسَلِّمْ وَبَارِكْ على نَبِيِّنَا مُحَمَّد.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "03",
        description: "",
        reference: "",
        content:
        "اللَّهُمَّ إِنَّا نَعُوذُ بِكَ مِنْ أَنْ نُشْرِكَ بِكَ شَيْئًا نَعْلَمُهُ ، وَنَسْتَغْفِرُكَ لِمَا لَا نَعْلَمُهُ.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "03",
        description: "",
        reference: "",
        content:
        "اللَّهُمَّ إِنِّي أَعُوذُ بِكَ مِنْ الْهَمِّ وَالْحَزَنِ، وَأَعُوذُ بِكَ مِنْ الْعَجْزِ وَالْكَسَلِ، وَأَعُوذُ بِكَ مِنْ الْجُبْنِ وَالْبُخْلِ، وَأَعُوذُ بِكَ مِنْ غَلَبَةِ الدَّيْنِ، وَقَهْرِ الرِّجَالِ.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "100",
        description:
        "كانت له عدل عشر رقاب، وكتبت له مئة حسنة، ومحيت عنه مئة سيئة، وكانت له حرزا من الشيطان.",
        reference: "",
        content:
        "لَا إلَه إلّا اللهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلُّ شَيْءِ قَدِيرِ.",
      ),
      Azkar(
        category: "أذكار المساء",
        count: "100",
        description:
        "حُطَّتْ خَطَايَاهُ وَإِنْ كَانَتْ مِثْلَ زَبَدِ الْبَحْرِ. لَمْ يَأْتِ أَحَدٌ يَوْمَ الْقِيَامَةِ بِأَفْضَلَ مِمَّا جَاءَ بِهِ إِلَّا أَحَدٌ قَالَ مِثْلَ مَا قَالَ أَوْ زَادَ عَلَيْهِ.",
        reference: "",
        content:
        "سُبْحـانَ اللهِ وَبِحَمْـدِهِ",
      ),
    ],
  ),
  // فئة "أذكار بعد السلام من الصلاة المفروضة"
  Category(
    title: "أذكار بعد السلام من الصلاة المفروضة",
    azkarList: [
      Azkar(
        category: "أذكار بعد السلام من الصلاة المفروضة",
        count: "01",
        description: "",
        reference: "",
        content:
        "أَسْـتَغْفِرُ الله، أَسْـتَغْفِرُ الله، أَسْـتَغْفِرُ الله.\nاللّهُـمَّ أَنْتَ السَّلامُ ، وَمِـنْكَ السَّلام ، تَبارَكْتَ يا ذا الجَـلالِ وَالإِكْـرام .",
      ),
      Azkar(
        category: "أذكار بعد السلام من الصلاة المفروضة",
        count: "01",
        description: "",
        reference: "",
        content:
        "لا إلهَ إلاّ اللّهُ وحدَهُ لا شريكَ لهُ، لهُ المُـلْكُ ولهُ الحَمْد، وهوَ على كلّ شَيءٍ قدير، اللّهُـمَّ لا مانِعَ لِما أَعْطَـيْت، وَلا مُعْطِـيَ لِما مَنَـعْت، وَلا يَنْفَـعُ ذا الجَـدِّ مِنْـكَ الجَـد.",
      ),
      Azkar(
        category: "أذكار بعد السلام من الصلاة المفروضة",
        count: "01",
        description: "",
        reference: "",
        content:
        "لا إلهَ إلاّ اللّه, وحدَهُ لا شريكَ لهُ، لهُ الملكُ ولهُ الحَمْد، وهوَ على كلّ شيءٍ قدير، لا حَـوْلَ وَلا قُوَّةَ إِلاّ بِاللهِ، لا إلهَ إلاّ اللّـه، وَلا نَعْـبُـدُ إِلاّ إيّـاه، لَهُ النِّعْـمَةُ وَلَهُ الفَضْل وَلَهُ الثَّـناءُ الحَـسَن، لا إلهَ إلاّ اللّهُ مخْلِصـينَ لَـهُ الدِّينَ وَلَوْ كَـرِهَ الكـافِرون.",
      ),
      Azkar(
        category: "أذكار بعد السلام من الصلاة المفروضة",
        count: "33",
        description: "",
        reference: "",
        content:
        "سُـبْحانَ اللهِ، والحَمْـدُ لله ، واللهُ أكْـبَر.",
      ),
      Azkar(
        category: "أذكار بعد السلام من الصلاة المفروضة",
        count: "01",
        description: "",
        reference: "",
        content:
        "لا إلهَ إلاّ اللّهُ وَحْـدَهُ لا شريكَ لهُ، لهُ الملكُ ولهُ الحَمْد، وهُوَ على كُلّ شَيْءٍ قَـدير.",
      ),
      Azkar(
        category: "أذكار بعد السلام من الصلاة المفروضة",
        count: "10",
        description: "عَشْر مَرّات بَعْدَ المَغْرِب وَالصّـبْح.",
        reference: "",
        content:
        "لا إلهَ إلاّ اللّهُ وحْـدَهُ لا شريكَ لهُ، لهُ المُلكُ ولهُ الحَمْد، يُحيـي وَيُمـيتُ وهُوَ على كُلّ شيءٍ قدير.",
      ),
      Azkar(
        category: "أذكار بعد السلام من الصلاة المفروضة",
        count: "01",
        description: "بَعْد السّلامِ من صَلاةِ الفَجْر.",
        reference: "",
        content:
        "اللّهُـمَّ إِنِّـي أَسْأَلُـكَ عِلْمـاً نافِعـاً وَرِزْقـاً طَيِّـباً ، وَعَمَـلاً مُتَقَـبَّلاً.",
      ),
      Azkar(
        category: "أذكار بعد السلام من الصلاة المفروضة",
        count: "07",
        description: "بعد صلاة الصبح والمغرب.",
        reference: "",
        content:
        "اللَّهُمَّ أَجِرْنِي مِنْ النَّار.",
      ),
      Azkar(
        category: "أذكار بعد السلام من الصلاة المفروضة",
        count: "01",
        description: "",
        reference: "",
        content:
        "اللَّهُمَّ أَعِنِّي عَلَى ذِكْرِكَ وَشُكْرِكَ وَحُسْنِ عِبَادَتِكَ.",
      ),
    ],
  ),
  // فئة "تسابيح"
  Category(
    title: "تسابيح",
    azkarList: [
      Azkar(
        category: "تسابيح",
        count: "100",
        description: "يكتب له ألف حسنة أو يحط عنه ألف خطيئة.",
        reference: "",
        content: "سُبْحَانَ اللَّهِ",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description:
        "حُطَّتْ خَطَايَاهُ وَإِنْ كَانَتْ مِثْلَ زَبَدِ الْبَحْرِ. لَمْ يَأْتِ أَحَدٌ يَوْمَ الْقِيَامَةِ بِأَفْضَلَ مِمَّا جَاءَ بِهِ إِلَّا أَحَدٌ قَالَ مِثْلَ مَا قَالَ أَوْ زَادَ عَلَيْهِ.",
        reference: "",
        content: "سُبْحَانَ اللَّهِ وَبِحَمْدِهِ",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description:
        "تَمْلَآَنِ مَا بَيْنَ السَّمَاوَاتِ وَالْأَرْضِ.",
        reference: "",
        content: "سُبْحَانَ اللَّهِ وَالْحَمْدُ لِلَّهِ",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description: "غرست له نخلة في الجنة.",
        reference: "",
        content: "سُبْحَانَ اللهِ العَظِيمِ وَبِحَمْدِهِ",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description:
        "ثقيلتان في الميزان حبيبتان إلى الرحمن.",
        reference: "",
        content:
        "سُبْحَانَ اللَّهِ وَبِحَمْدِهِ ، سُبْحَانَ اللَّهِ الْعَظِيمِ",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description:
        "كانت له عدل عشر رقاب، وكتبت له مئة حسنة، ومحيت عنه مئة سيئة، وكانت له حرزا من الشيطان.",
        reference: "",
        content:
        "لَا إلَه إلّا اللهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلُّ شَيْءِ قَدِيرِ.",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description: "كنز من كنوز الجنة.",
        reference: "",
        content: "لا حَوْلَ وَلا قُوَّةَ إِلا بِاللَّهِ",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description: "تملأ ميزان العبد بالحسنات.",
        reference: "",
        content: "الْحَمْدُ للّهِ رَبِّ الْعَالَمِينَ",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description:
        "من صلى على حين يصبح وحين يمسى ادركته شفاعتى يوم القيامة.",
        reference: "",
        content:
        "الْلَّهُم صَلِّ وَسَلِّم وَبَارِك عَلَى سَيِّدِنَا مُحَمَّد",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description: "لفعل الرسول صلى الله عليه وسلم.",
        reference: "",
        content: "أستغفر الله",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description:
        "أنهن أحب الكلام الى الله، ومكفرات للذنوب، وغرس الجنة، وجنة لقائلهن من النار، وأحب الى النبي عليه السلام مما طلعت عليه الشمس، والْبَاقِيَاتُ الْصَّالِحَات.",
        reference: "",
        content:
        "سُبْحَانَ الْلَّهِ، وَالْحَمْدُ لِلَّهِ، وَلَا إِلَهَ إِلَّا الْلَّهُ، وَالْلَّهُ أَكْبَرُ",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description: "أفضل الذكر لا إله إلاّ الله.",
        reference: "",
        content: "لَا إِلَهَ إِلَّا اللَّهُ",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description:
        "من قال الله أكبر كتبت له عشرون حسنة وحطت عنه عشرون سيئة. الله أكبر من كل شيء.",
        reference: "",
        content: "الْلَّهُ أَكْبَرُ",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description: "خير الدنيا والآخرة",
        reference: "",
        content:
        "سُبْحَانَ اللَّهِ ، وَالْحَمْدُ لِلَّهِ ، وَلا إِلَهَ إِلا اللَّهُ ، وَاللَّهُ أَكْبَرُ ، اللَّهُمَّ اغْفِرْ لِي ، اللَّهُمَّ ارْحَمْنِي ، اللَّهُمَّ ارْزُقْنِي.",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description:
        "قَالَ النَّبِيُّ صَلَّى اللَّهُ عَلَيْهِ وَسَلَّمَ \"عَجِبْتُ لَهَا ، فُتِحَتْ لَهَا أَبْوَابُ السَّمَاءِ\".",
        reference: "",
        content:
        "اللَّهُ أَكْبَرُ كَبِيرًا ، وَالْحَمْدُ لِلَّهِ كَثِيرًا ، وَسُبْحَانَ اللَّهِ بُكْرَةً وَأَصِيلاً.",
      ),
      Azkar(
        category: "تسابيح",
        count: "100",
        description:
        "في كل مره تحط عنه عشر خطايا ويرفع له عشر درجات ويصلي الله عليه عشرا وتعرض على الرسول صلى الله عليه وسلم.",
        reference: "",
        content:
        "اللَّهُمَّ صَلِّ عَلَى مُحَمَّدٍ وَعَلَى آلِ مُحَمَّدٍ كَمَا صَلَّيْتَ عَلَى إِبْرَاهِيمَ , وَعَلَى آلِ إِبْرَاهِيمَ إِنَّكَ حَمِيدٌ مَجِيدٌ , اللَّهُمَّ بَارِكْ عَلَى مُحَمَّدٍ وَعَلَى آلِ مُحَمَّدٍ كَمَا بَارَكْتَ عَلَى إِبْرَاهِيمَ وَعَلَى آلِ إِبْرَاهِيمَ إِنَّكَ حَمِيدٌ مَجِيدٌ.",
      ),
    ],
  ),
  // فئة "أذكار النوم"
  Category(
    title: "أذكار النوم",
    azkarList: [
      Azkar(
        category: "أذكار النوم",
        count: "01",
        description: "",
        reference: "",
        content:
        "بِاسْمِكَ رَبِّـي وَضَعْـتُ جَنْـبي ، وَبِكَ أَرْفَعُـه، فَإِن أَمْسَـكْتَ نَفْسـي فارْحَـمْها ، وَإِنْ أَرْسَلْتَـها فاحْفَظْـها بِمـا تَحْفَـظُ بِه عِبـادَكَ الصّـالِحـين.",
      ),
      Azkar(
        category: "أذكار النوم",
        count: "01",
        description: "",
        reference: "",
        content:
        "اللّهُـمَّ إِنَّـكَ خَلَـقْتَ نَفْسـي وَأَنْـتَ تَوَفّـاهـا لَكَ ممَـاتـها وَمَحْـياها ، إِنْ أَحْيَيْـتَها فاحْفَظْـها ، وَإِنْ أَمَتَّـها فَاغْفِـرْ لَـها . اللّهُـمَّ إِنَّـي أَسْـأَلُـكَ العـافِـيَة.",
      ),
      Azkar(
        category: "أذكار النوم",
        count: "03",
        description: "",
        reference: "",
        content:
        "اللّهُـمَّ قِنـي عَذابَـكَ يَـوْمَ تَبْـعَثُ عِبـادَك.",
      ),
      Azkar(
        category: "أذكار النوم",
        count: "01",
        description: "",
        reference: "",
        content: "بِاسْمِكَ اللّهُـمَّ أَمـوتُ وَأَحْـيا.",
      ),
      Azkar(
        category: "أذكار النوم",
        count: "01",
        description: "",
        reference: "",
        content:
        "الـحَمْدُ للهِ الَّذي أَطْـعَمَنا وَسَقـانا، وَكَفـانا، وَآوانا، فَكَـمْ مِمَّنْ لا كـافِيَ لَـهُ وَلا مُـؤْوي.",
      ),
      Azkar(
        category: "أذكار النوم",
        count: "01",
        description: "",
        reference: "",
        content:
        "اللّهُـمَّ عالِـمَ الغَـيبِ وَالشّـهادةِ فاطِـرَ السّماواتِ وَالأرْضِ رَبَّ كُـلِّ شَـيءٍ وَمَليـكَه، أَشْهـدُ أَنْ لا إِلـهَ إِلاّ أَنْت، أَعـوذُ بِكَ مِن شَـرِّ نَفْسـي، وَمِن شَـرِّ الشَّيْـطانِ وَشِـرْكِه، وَأَنْ أَقْتَـرِفَ عَلـى نَفْسـي سوءاً أَوْ أَجُـرَّهُ إِلـى مُسْـلِم .",
      ),
      Azkar(
        category: "أذكار النوم",
        count: "01",
        description: "",
        reference: "",
        content:
        "اللّهُـمَّ أَسْـلَمْتُ نَفْـسي إِلَـيْكَ، وَفَوَّضْـتُ أَمْـري إِلَـيْكَ، وَوَجَّـهْتُ وَجْـهي إِلَـيْكَ، وَأَلْـجَـاْتُ ظَهـري إِلَـيْكَ، رَغْبَـةً وَرَهْـبَةً إِلَـيْكَ، لا مَلْجَـأَ وَلا مَنْـجـا مِنْـكَ إِلاّ إِلَـيْكَ، آمَنْـتُ بِكِتـابِكَ الّـذي أَنْزَلْـتَ وَبِنَبِـيِّـكَ الّـذي أَرْسَلْـت.",
      ),
      Azkar(
        category: "أذكار النوم",
        count: "33",
        description: "",
        reference: "",
        content: "سُبْحَانَ اللَّهِ.",
      ),
      Azkar(
        category: "أذكار النوم",
        count: "33",
        description: "",
        reference: "",
        content: "الْحَمْدُ لِلَّهِ.",
      ),
      Azkar(
        category: "أذكار النوم",
        count: "34",
        description: "",
        reference: "",
        content: "اللَّهُ أَكْبَرُ.",
      ),
    ],
  ),
  // فئة "أذكار الاستيقاظ"
  Category(
    title: "أذكار الاستيقاظ",
    azkarList: [
      Azkar(
        category: "أذكار الاستيقاظ",
        count: "01",
        description: "",
        reference: "",
        content:
        "الحَمْـدُ لِلّهِ الّذي أَحْـيانا بَعْـدَ ما أَماتَـنا وَإليه النُّشُور.",
      ),
      Azkar(
        category: "أذكار الاستيقاظ",
        count: "01",
        description: "",
        reference: "",
        content:
        "الحمدُ للهِ الذي عافاني في جَسَدي وَرَدّ عَليّ روحي وَأَذِنَ لي بِذِكْرِه.",
      ),
      Azkar(
        category: "أذكار الاستيقاظ",
        count: "01",
        description: "",
        reference: "",
        content:
        "لا إلهَ إلاّ اللّهُ وَحْـدَهُ لا شَـريكَ له، لهُ المُلـكُ ولهُ الحَمْد، وهوَ على كلّ شيءٍ قدير، سُبْحانَ اللهِ، والحمْـدُ لله ، ولا إلهَ إلاّ اللهُ واللهُ أكبَر، وَلا حَولَ وَلا قوّة إلاّ باللّهِ العليّ العظيم. رَبِّ اغْفرْ لي.",
      ),
    ],
  ),
  // فئة "أدعية قرآنية"
  Category(
    title: "أدعية قرآنية",
    azkarList: [
      Azkar(
        category: "أدعية قرآنية",
        count: "01",
        description: "",
        reference: ". [البقرة - 201].",
        content: "رَبَّنَا آتِنَا فِي الدُّنْيَا ...",
      ),
      Azkar(
        category: "أدعية قرآنية",
        count: "01",
        description: "",
        reference: ". [البقرة - 250].",
        content: "رَبَّنَا أَفْرِغْ عَلَيْنَا ...",
      ),
      Azkar(
        category: "أدعية قرآنية",
        count: "01",
        description: "",
        reference: ". [البقرة - 286].",
        content: "رَبَّنَا لاَ تُؤَاخِذْنَا ...",
      ),
      Azkar(
        category: "أدعية قرآنية",
        count: "01",
        description: "",
        reference: ". [آل عمران - 8].",
        content: "رَبَّنَا لاَ تُزِغْ قُلُوبَنَا ...",
      ),
      Azkar(
        category: "أدعية قرآنية",
        count: "01",
        description: "",
        reference: ". [آل عمران - 16].",
        content: "رَبَّنَا إِنَّنَا آمَنَّا ...",
      ),
      Azkar(
        category: "أدعية قرآنية",
        count: "01",
        description: "",
        reference: ". [آل عمران - 38].",
        content: "رَبِّ هَبْ لِي مِن لَّدُنْكَ ...",
      ),
      Azkar(
        category: "أدعية قرآنية",
        count: "01",
        description: "",
        reference: ". [آل عمران - 53].",
        content: "رَبَّنَا آمَنَّا بِمَا أَنزَلْتَ ...",
      ),
      Azkar(
        category: "أدعية قرآنية",
        count: "01",
        description: "",
        reference: ". [آل عمران - 147].",
        content: "رَبَّنَا اغْفِرْ لَنَا ...",
      ),
      Azkar(
        category: "أدعية قرآنية",
        count: "01",
        description: "",
        reference: ". [الأعراف - 23].",
        content: "رَبَّنَا ظَلَمْنَا أَنفُسَنَا ...",
      ),
      Azkar(
        category: "أدعية قرآنية",
        count: "01",
        description: "",
        reference: ". [الأعراف - 47].",
        content: "رَبَّنَا لاَ تَجْعَلْنَا ...",
      ),
    ],
  ),
  // فئة "أدعية الأنبياء"
  Category(
    title: "أدعية الأنبياء",
    azkarList: [
      Azkar(
        category: "أدعية الأنبياء",
        count: "01",
        description: "آدم علية السلام.",
        reference: ". [الأعراف - 23].",
        content: "رَبَّنَا ظَلَمْنَا أَنفُسَنَا ...",
      ),
      Azkar(
        category: "أدعية الأنبياء",
        count: "01",
        description: "نوح علية السلام.",
        reference: ". [نوح - 28].",
        content: "رَبِّ اغْفِرْ لِي وَلِوَالِدَيَّ ...",
      ),
      Azkar(
        category: "أدعية الأنبياء",
        count: "01",
        description: "إبراهيم علية السلام.",
        reference: ". [هود - 47].",
        content: "رَبَّنَا تَقَبَّلْ مِنَّا ...",
      ),
      // عناصر إضافية لإكمال قائمة "أدعية الأنبياء"
      Azkar(
        category: "أدعية الأنبياء",
        count: "01",
        description: "دعاء إضافي.",
        reference: ". [مثال].",
        content: "رَبِّ اجْعَلْنِي مِن الصّالِحِينَ.",
      ),
      Azkar(
        category: "أدعية الأنبياء",
        count: "01",
        description: "دعاء إضافي.",
        reference: ". [مثال].",
        content: "اللهم ارزقني الإيمان والثبات.",
      ),
      Azkar(
        category: "أدعية الأنبياء",
        count: "01",
        description: "دعاء إضافي.",
        reference: ". [مثال].",
        content: "رَبِّ اغْفِرْ لَنَا ذُنُوبَنَا.",
      ),
      Azkar(
        category: "أدعية الأنبياء",
        count: "01",
        description: "دعاء إضافي.",
        reference: ". [مثال].",
        content: "اللهم اجعلنا من ورثة جنتك.",
      ),
      Azkar(
        category: "أدعية الأنبياء",
        count: "01",
        description: "دعاء إضافي.",
        reference: ". [مثال].",
        content: "رَبِّ زِدْنِي عِلْمًا.",
      ),
      Azkar(
        category: "أدعية الأنبياء",
        count: "01",
        description: "دعاء إضافي.",
        reference: ". [مثال].",
        content: "اللهم ارزقنا التوبة والإنابة.",
      ),
      Azkar(
        category: "أدعية الأنبياء",
        count: "01",
        description: "دعاء إضافي.",
        reference: ". [مثال].",
        content: "رَبَّنَا آتِنَا مِن لَّدُنكَ رَحْمَةً.",
      ),
    ],
  ),
];

final List<Azkar> azkar = categories.expand((cat) => cat.azkarList).toList();
